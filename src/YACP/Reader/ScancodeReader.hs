  ...
  -- let idFromHashes = mconcat $ sha1 ++ md5 ++ sha256 ++ sha512
    ...
    license <-
      ...
      >>=   ( return
            . (\case
                Just lics ->
                  mconcat (map (fromString . licenseTransformator) lics)
...
convertScancode (ScancodeFile _ files) =
  let convertScancodeFilePackages
        :: Statemental a
        => Identifier
        -> (Identifier -> a)
        -> ScancodePackage
        -> Statements
      convertScancodeFilePackages fileIdentifier fun scp =
        case (_scp_purl scp) of
          Just purl ->
            let packageIdentifier  = PurlIdentifier purl
                packagesStatements = mconcat $ map
                  (convertScancodeFilePackages packageIdentifier FoundDependent)
                  (_scp_dependencies scp)
            in  (Statements $ V.fromList
                  [ Statement packageIdentifier
                              (ComponentLicense (_scp_licenses scp))
          ...
          Nothing ->
            Statements
              $ V.fromList
                  [ Statement fileIdentifier
                              (ComponentLicense (_scp_licenses scp))
                  ]
      convertScancodeFile :: ScancodeFileEntry -> Statements
      convertScancodeFile scfe =
        let fileIdentifier     = AbsolutePathIdentifier (_scfe_file scfe)
            fileLicenses       = DetectedLicenses [_scfe_license scfe]
            packagesStatements = mconcat $ map
              (convertScancodeFilePackages fileIdentifier FoundManifestFile)
              (_scfe_packages scfe)
        in  (Statements $ V.fromList [Statement fileIdentifier fileLicenses])
              <> packagesStatements
...
readScancodeFile f =
  readBSFromFile (readScancodeBS (OriginToolReport "scancode" f)) f
