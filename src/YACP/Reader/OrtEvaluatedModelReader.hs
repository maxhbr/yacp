...
data EvaluatedModelPackage = EvaluatedModelPackage
...
data EvaluatedModelDependencyTree = EvaluatedModelDependencyTree
...
data EvaluatedModelFile = EvaluatedModelFile
...
instance A.FromJSON EvaluatedModelFile where
  parseJSON =
    let
      parseToMap
        :: (A.Object -> A.Parser a) -> A.Value -> A.Parser (Map.Map Int a)
      parseToMap parser = A.withArray "Array" $ \v -> do
        entries <- mapM
          ( A.withObject "ArrayItem"
          $ \v' -> (,) <$> v' A..: "_id" <*> parser v'
          )
          (V.toList v)
        return $ Map.fromList entries
      parseSimpleToMap
        :: A.FromJSON a => A.Key -> A.Value -> A.Parser (Map.Map Int a)
      ...
      parsePackage
        :: (Map.Map Int MaybeLicenseExpression)
        -> (Map.Map Int String)
        -> A.Object
        -> A.Parser EvaluatedModelPackage
      parsePackage licenseMap scopeMap = \v -> do
        EvaluatedModelPackage
          ...
          <*>  (    v
               A..: "declared_licenses_processed"
               >>=  (\v' -> do
                      spdxExp <-
                        (v' A..:? "spdx_expression" >>= return . \case
                          Just exp -> fromString exp
                          Nothing  -> MLicExp NOASSERTION
                        )
                      unmapped <-
                        (([] `fromMaybe`) <$> v' A..:? "unmapped_licenses")
                        >>= return
                        .   lookupsInMap licenseMap
      ...
      parsePackageMap
        ...
        -> (Map.Map Int String)
        -> A.Value
        -> A.Parser (Map.Map Int EvaluatedModelPackage)
      parsePackageMap licenseMap scopeMap =
        parseToMap (parsePackage licenseMap scopeMap)

      parseDependencyTree
        :: (Map.Map Int String)
        -> (Map.Map Int EvaluatedModelPackage)
        -> A.Value
        -> A.Parser EvaluatedModelDependencyTree
      parseDependencyTree scopeMap packageMap =
        A.withObject "EvaluatedModelDependencyTree" $ \v ->
          EvaluatedModelDependencyTree
            ...
            <*>   v
            A..:? "linkage"
      parseDependencyTrees
        :: (Map.Map Int String)
        -> (Map.Map Int EvaluatedModelPackage)
        -> A.Value
        -> A.Parser [EvaluatedModelDependencyTree]
      parseDependencyTrees scopeMap packageMap = A.withArray "Array"
        $ \v -> mapM (parseDependencyTree scopeMap packageMap) (V.toList v)
    in
      A.withObject "EvaluatedModelFile" $ \v -> do
        licenseMap <-
          (v A..: "licenses" >>= parseSimpleToMap "id") :: A.Parser
            (Map.Map Int MaybeLicenseExpression)
        scopeMap <-
          (v A..: "scopes" >>= parseSimpleToMap "name") :: A.Parser
            (Map.Map Int String)
        copyrightMap <-
          (v A..: "copyrights" >>= parseSimpleToMap "statement") :: A.Parser
            (Map.Map Int String)

        packageMap <-
          (v A..: "packages" >>= parsePackageMap licenseMap scopeMap) :: A.Parser
            (Map.Map Int EvaluatedModelPackage)

        dependencyTrees <-
          (    v
          A..: "dependency_trees"
          >>=  parseDependencyTrees scopeMap packageMap
          ) :: A.Parser [EvaluatedModelDependencyTree]

        return
          (EvaluatedModelFile licenseMap
                              scopeMap
                              copyrightMap
                              packageMap
                              dependencyTrees
...
convertEvaluatedModel (EvaluatedModelFile { _evaluatedModelPackages = packages })
  = let
      convertEvaluatedPackage :: EvaluatedModelPackage -> [Statement]
      convertEvaluatedPackage p =
        let
          identifier =
            Identifier (_evaluatedModelPackageId p)
              <> (case _evaluatedModelPackagePurl p of
                   Just p  -> PurlIdentifier p
                   Nothing -> mempty
                 )
          detectedLicenses = Statement
            identifier
            (DetectedLicenses (_evaluatedModelPackageDetectedLicenses p))
          declaredLicenses = Statement
            identifier
            (ComponentLicense (_evaluatedModelPackageDeclaredLicenses p))
          manifestFile = case _evaluatedModelPackageDefinitionFilePath p of
            Just manifest ->
              [ Statement
                  identifier
                  (FoundManifestFile (AbsolutePathIdentifier manifest))
              ]
            Nothing -> []
        in
          [declaredLicenses, detectedLicenses] ++ manifestFile
    in
      (Statements . V.fromList . concatMap convertEvaluatedPackage . Map.elems)
        packages
...
readEvaluatedModelFile f =
  readBSFromFile (readEvaluatedModelBS (OriginToolReport "ort" f)) f
