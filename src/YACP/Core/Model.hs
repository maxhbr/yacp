...
refinePurl = let
    ...
    applyGuessType (p@PURL{_PURL_namespace = Nothing, _PURL_name = name}) = case guessType name of
      Just (ty, name') -> p{_PURL_type = Just ty, _PURL_name = name'}
      Nothing -> p
    applyGuessType (p@PURL{_PURL_namespace = Just namespace}) = case guessType namespace of
      Just (ty, namespace') -> p{_PURL_type = Just ty, _PURL_namespace = Just namespace'}
      Nothing -> p
    ...
    applyGuessNamespaceFromName (p@PURL{_PURL_namespace = Nothing, _PURL_name = name}) = case guessNamespaceFromName name of
      Just (namespace, name') -> p{_PURL_namespace = Just namespace, _PURL_name = name'}
      Nothing -> p
