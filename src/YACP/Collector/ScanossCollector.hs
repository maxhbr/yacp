...
instance A.FromJSON ScanossFindingLicense where
  parseJSON = A.withObject "ScanossFindingLicense" $ \v -> 
      ScanossFindingLicense <$> v A..: "name"
                            <*> v A..: "source"
...
-- }
data ScanossFindingQuality
  = ScanossFindingQuality
  { _SCF_Q_score :: String
  , _SCF_Q_source :: String
  } deriving (Eq, Show)
instance A.FromJSON ScanossFindingQuality where
  parseJSON = A.withObject "ScanossFindingQuality" $ \v -> 
      ScanossFindingQuality <$> v A..: "score"
                            <*> v A..: "source"
...
instance A.FromJSON ScanossFinding where
  parseJSON = A.withObject "ScanossFinding" $ \v -> 
      undefined

data ScanossFile
  = ScanossFile (Map.Map FilePath [ScanossFinding])
