...
data CSVRecord = CSVRecord Identifier [(ComponentLicense,Origin)] [(ComponentVulnerability,Origin)] [Origin]

instance Csv.ToNamedRecord CSVRecord where
  toNamedRecord (CSVRecord identifier lics vulns origins) = let
    toListOfStrings :: Show a => [(a,Origin)] -> String
    toListOfStrings = let
        fun :: Show a => (a,Origin) -> String
        fun (a,o) = show a ++ " via " ++ show o
      in unlines . map fun
    in Csv.namedRecord ["Identifier" Csv..= (unlines . map show . flattenIdentifierToList) identifier
